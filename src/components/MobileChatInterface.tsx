...
import { Send, File, MoreHorizontal, Menu, Languages, RefreshCw, Bot, User } from 'lucide-react';
...
const MobileChatInterface = ({ documentName, sessionId }: MobileChatInterfaceProps) => {
  ...
  const { saveMessage, restoreChat, initializeSession, generateResponse } = useChat();
    ...
    setMessages(prev =>
      prev.map(msg => {
        if (msg.id === messageId && !msg.isUser) {
          ...
          const translatedContent = `[ES] ${msg.content}`;
          return {
            ...msg,
            translatedContent,
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            {messages.map(message => (
              <div key={message.id} className={`flex gap-4 ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-4 py-3 ${message.isUser ? 'bg-chat-user text-chat-user-foreground' : 'bg-chat-ai text-chat-ai-foreground'}`}>
                  {message.isUser ? (
                    <p className="text-sm leading-relaxed">{message.content}</p>
                  ) : (
                    <MarkdownRenderer content={message.showTranslation ? message.translatedContent || message.content : message.content} />
                  )}

                  {!message.isUser && (
                    <div className="mt-2 flex items-center gap-2">
                      <Button variant="ghost" size="sm" onClick={() => handleTranslate(message.id)} className="h-6 px-2 text-xs">
        ...
        <div className="p-4 border-t border-border bg-card/50 backdrop-blur-sm">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-end gap-3">
              ...
              <Button onClick={handleSendMessage} disabled={!inputMessage.trim() || isTyping} size="lg" className="h-[60px] w-[60px] rounded-full">
